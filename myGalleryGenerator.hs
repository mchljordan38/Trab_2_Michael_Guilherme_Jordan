...
genContent items = foldl1 (++)[imageDiv x | x <- items] -- aplica imageDiv para cada item da lista e então concatena todos os elementos da mesma, gerando uma String de saída utilizando foldl1
-- foldl1 pega os dois primeiros itens de uma lista e aplica a função sugerida. Após isto o resultado da primeira operação é utilizado para a realização da mesma operação com o terceiro elemento e assim sucessivamente.
